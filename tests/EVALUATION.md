# Evaluating agent authoring ability

This is for maintainers comparing models or skill revisions. It is not needed to
author a deck; the delivery gates live in
[verification and delivery](../references/verification-and-delivery.md).

Field outputs must be compared with the task, data and requested reading mode, not
just gallery screenshots. No external-model performance claim follows from local
regressions.

The briefs in [agent-evals.json](agent-evals.json) and
[transfer-evals.json](transfer-evals.json) are reusable tests for *another* model.
